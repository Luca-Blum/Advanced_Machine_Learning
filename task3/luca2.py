...
signal_cwt, waves_cwt = nk.ecg_delineate(ecg_signal, rpeaks, sampling_rate=300,
                                         method="cwt", show=True, show_type='all')

# Visualize P-peaks and T-peaks
signal_cwt, waves_cwt = nk.ecg_delineate(ecg_signal, rpeaks, sampling_rate=300,
                                         method="cwt", show=True, show_type='peaks')

# Visualize T-waves boundaries
signal_cwt, waves_cwt = nk.ecg_delineate(ecg_signal, rpeaks, sampling_rate=300,
                                         method="cwt", show=True, show_type='bounds_T')

# Visualize P-waves boundaries
signal_cwt, waves_cwt = nk.ecg_delineate(ecg_signal, rpeaks, sampling_rate=300,
                                         method="cwt", show=True, show_type='bounds_P')

# Visualize R-waves boundaries
signal_cwt, waves_cwt = nk.ecg_delineate(ecg_signal, rpeaks, sampling_rate=300,
                                         method="cwt", show=True, show_type='bounds_R')
...
if len(R_amplitudes) > 4:
    ...
    # remove nan values
    Q_amplitudes = [ecg_signal[peak_index] if str(peak_index) != 'nan' else - np.infty
                    for peak_index in waves_peak['ECG_Q_Peaks']]

    print(R_amplitudes)
    print(Q_amplitudes)

    print([1 if np.abs(rpeak) > np.abs(Q_amplitudes[index]) else -1 for index, rpeak in enumerate(R_amplitudes)])
    if np.sum([1 if np.abs(rpeak) > np.abs(Q_amplitudes[index]) else -1
               for index, rpeak in enumerate(R_amplitudes)]) < 0:
        print("flip")
        ecg_signal = -ecg_signal
...
for i in range(y_plots):
    for j in range(x_plots):
        ...
        axs[i, j].fill_between(out['templates_ts'],
                               mean.astype(np.float64) + std,
                               mean.astype(np.float64) - std, alpha=0.2)
